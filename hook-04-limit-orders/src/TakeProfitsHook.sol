...
contract TakeProfitsHook is BaseHook, ERC1155 {
    ...
    // Last tick that was used for a take-profit order
        ...
        returns (bool tryMore, int24 newTick)
    {
        ...
        if (currentTick > lastTick) {
            // Loop over all usable ticks from `lastTick` up to `currentTick` and
            ...
            for (
                int24 tick = getLowerUsableTick(lastTick, key.tickSpacing); tick < currentTick; tick += key.tickSpacing
            ) {
