...
contract TestPointsHook is Test, Deployers, ERC1155TokenReceiver {
    ...
    function test_swap_success() public {
        ...
        // get the original points balance for the user using the pool id
        uint256 pointsBalanceOriginal = hook.balanceOf(address(this), poolIdUint);

        // now we can confirm the original points balance is 0
        assertEq(pointsBalanceOriginal, 0, "Points balance should be 0");
        ...
        // check that we will emit the PointsAllocated event
        vm.expectEmit(true, true, true, true);
        emit PointsHook.PointsAllocated(address(this), key.toId(), 2 * 10 ** 14);
        ...
        assertEq(pointsBalanceAfterSwap - pointsBalanceOriginal, 0.001 / 5 * 10 ** 18, "Points balance is not correct");
    }
    ...
    function test_swap_token_for_eth_no_points() public {
        ...
        // Now we swap
        // We will swap 0.001 tokens for eth
        // We should not get any points
        swapRouter.swap(
            key,
            SwapParams({
                zeroForOne: false,
        ...
        uint256 pointsBalanceAfterSwap = hook.balanceOf(address(this), poolIdUint);
        assertEq(pointsBalanceAfterSwap, pointsBalanceOriginal, "Points balance should be 0");
    }
    // TODO: Uncomment when intialize pool with currency0 - not ETH
    /*function test_swap_token_currency0notETH() public {
        // get the pool id
        uint256 poolIdUint = uint256(PoolId.unwrap(key.toId()));
        // get the original points balance for the user using the pool id
        uint256 pointsBalanceOriginal = hook.balanceOf(address(this), poolIdUint);
        ...
        // Now we swap
        // We will swap 0.001 tokens for eth
        // We should not get any points because currency0 is not ETH
        swapRouter.swap(
            key,
            SwapParams({
                zeroForOne: false,
    ...
    function test_swap_token_no_hook_data() public {
        // get the pool id
        uint256 poolIdUint = uint256(PoolId.unwrap(key.toId()));
        // get the original points balance for the user using the pool id
        uint256 pointsBalanceOriginal = hook.balanceOf(address(this), poolIdUint);
    ...
    function testfuzz_swap(address _pointsRecipient, uint256 _amount, bool _zeroForOne) public {
        // make sure amt <= uint128(max)  so we can convert the uint256 to an int256 without overflow
        // vm.assume(_amount < type(uint128).max);
        _amount = bound(_amount, 5 ether, type(uint128).max);
        vm.assume(_pointsRecipient != address(0));
        vm.assume(_pointsRecipient != address(vm));
        vm.assume(_pointsRecipient.code.length == 0);

        // we need to have  enough tokens to fill the swap
        if (_zeroForOne) {
            // we need to have enough ETH to fill the swap
            deal(address(this), _amount);
        } else {
            // we need to have enough TOKEN to fill the swap
        ...
        // = 2 * 10**14
        /* vm.expectEmit(true, true, true, true);
        emit PointsHook.PointsAllocated(_pointsRecipient, key.toId(), 2 * 10 ** 14);*/
