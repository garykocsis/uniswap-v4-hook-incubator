 ...
 * interacted with, to promote liquidity
 *
...
contract InternalSwapPool is BaseHook {
     ...
     * @param key The key for the pool
     * @param params The parameters for the swap
     * @param delta Tshe amount owed to the caller (positive) or owed to the pool (negative)
     * @param hookData Arbitrary data handed into the PoolManager by the swapper to be be passed on to the hook
     *
     * @return selector_ The function selector for the hook
        ...
        returns (bytes4 selector_, int128 hookDeltaUnspecified_)
    {
        ...
        uint256 swapFee = uint256(uint128(swapAmount)) * 1 / 100;
        ...
        _depositFees(key, params.zeroForOne ? 0 : swapFee, params.zeroForOne ? swapFee : 0);
    ...
    function _distributeFees(PoolKey calldata _poolKey) internal {
        ...
        // Settle the tokens from the contract
